...
class LiveTradingModel:
    ...
    def calculate_market_regime(self):
        ...
        stock_counter = 0
        latest_daily_date = self.daily_data['date'].iloc[-1]
    ...
    def trade_logic(self):
        ...
        for symbol in self.live_data['symbol'].unique():
            try:
                ...
                if (current_time - last_trade).dt.days < self.cool_down_period:
                    continue
                ...
                # Stop-Loss Sell
                    ...
                    if current_price < stop_loss_price:
                        ...
                        if sell_response['order_status'] == 'completed': 
                            ...
                            print(f"Stop-loss: Sold {symbol} at {sell_response['price']:.2f}")
                    ...
                    elif (current_time - self.last_trade_time[symbol]).dt.days > self.holding_period:
                        sell_response = lh.sell(ticker=symbol, amount=quantity)  
                        if sell_response['order_status'] == 'completed': 
                            self.cash_capital += quantity * sell_response['price']
                            self.portfolio_value -= quantity * sell_response['price']
        ...
        print(f"Divergence total value: ${self.cash_capital + self.portfolio_value:,.2f}")
